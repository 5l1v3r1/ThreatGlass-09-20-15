	...
	function showPreview(newSrc,imageIndex,tLink)
	{
		if(!previewImage){
			var images = document.getElementById('previewPane').getElementsByTagName('IMG');
			var imagesLink = document.getElementById('previewPane').getElementsByTagName('A');
			if(images.length>0){
				previewImage = images[0];
				previewLink = imagesLink[0];
			}else{
				previewImage = document.createElement('IMG');
				previewLink = document.createElement('A');
				document.getElementById('previewPane').appendChild(previewImage);	
				document.getElementById('previewPane').appendChild(previewLink);	
			}
		}
		...
		//alert(previewLink);
		previewLink.setAttribute('href',tLink);
